...
public class GreenCake  extends GameObjectImpl implements IPic{
	...
	public void draw() {
		try {
			this.setPath("/greenCake.png");
			if(images.get("greenCake")!=null)
				spriteImages[0]=(BufferedImage) images.get("greenCake");
			else {
				spriteImages[0] = ImageIO.read(getClass().getResourceAsStream(this.getPath()));
				images.put("greenCake",spriteImages[0] );
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
